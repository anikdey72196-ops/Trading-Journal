...
def set_daily_target():
    ...
    yesterday_stats = db.session.query(
        db.func.sum(Trades.trade_pnl),
        db.func.count(Trades.id)
    ).filter(
        Trades.user_id == user.id,
    ...
    yesterday_pnl = yesterday_stats[0] if yesterday_stats and yesterday_stats[0] is not None else 0.0
    yesterday_volume = yesterday_stats[1] if yesterday_stats and yesterday_stats[1] is not None else 0
